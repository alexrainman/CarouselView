...
namespace CarouselView.FormsPlugin.iOS
{
    ...
    public class CarouselViewRenderer : ViewRenderer<CarouselViewControl, UIView>
    {
        ...
        UIButton nextBtn;
        UIStackView buttons;
        UIButton prevTextBtn;
        UIButton nextTextBtn;
        NSLayoutConstraint[] center;
        NSLayoutConstraint[] alignLeft;
        NSLayoutConstraint[] alignRight;
        ...
        protected override void OnElementPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            switch (e.PropertyName)
            {
                ...
                case "ButtonPrevText":
                    ...
                    prevTextBtn.SetTitle(Element.ButtonPrevText, UIControlState.Normal);
                    break;
                case "ButtonNextText":
                    if (nextTextBtn == null) return;
                    nextTextBtn.SetTitle(Element.ButtonNextText, UIControlState.Normal);
                    break;
                case "ButtonsBackgroundColor":
                    if (prevTextBtn == null || nextTextBtn == null) return;
                    prevTextBtn.BackgroundColor = Element.ButtonsBackgroundColor.ToUIColor();
                    nextTextBtn.BackgroundColor = Element.ButtonsBackgroundColor.ToUIColor();
                    break;
                case "ButtonsTextColor":
                    if (prevTextBtn == null || nextTextBtn == null) return;
                    prevTextBtn.SetTitleColor(Element.ButtonsTextColor.ToUIColor(), UIControlState.Normal);
                    nextTextBtn.SetTitleColor(Element.ButtonsTextColor.ToUIColor(), UIControlState.Normal);
                    break;
            }
        }
        ...
        void SetButtons()
        {
            ...
            if (Element.ShowButtons)
            {
                ...
                prevTextBtn = new UIButton();
                prevTextBtn.BackgroundColor = Element.ButtonsBackgroundColor.ToUIColor();
                prevTextBtn.SetTitle(Element.ButtonPrevText, UIControlState.Normal);
                prevTextBtn.SetTitleColor(Element.ButtonsTextColor.ToUIColor(), UIControlState.Normal);
                prevTextBtn.TouchUpInside += PrevBtn_TouchUpInside;

                nextTextBtn = new UIButton();
                nextTextBtn.BackgroundColor = Element.ButtonsBackgroundColor.ToUIColor();
                nextTextBtn.SetTitle(Element.ButtonNextText, UIControlState.Normal);
                nextTextBtn.SetTitleColor(Element.ButtonsTextColor.ToUIColor(), UIControlState.Normal);
                nextTextBtn.TouchUpInside += NextBtn_TouchUpInside;

                buttons = new UIStackView(new UIView[] { prevTextBtn, nextTextBtn });
                buttons.Axis = UILayoutConstraintAxis.Horizontal;
                buttons.Alignment = UIStackViewAlignment.Fill;
                buttons.Distribution = UIStackViewDistribution.FillEqually;
                buttons.Spacing = 20;
                buttons.TranslatesAutoresizingMaskIntoConstraints = false;

                pageController.View.AddSubview(buttons);
                var spaceHorizontal = (ElementWidth + buttons.Spacing) / 2;
                var btnsDictionary = NSDictionary.FromObjectsAndKeys(new NSObject[] { pageController.View, buttons }, new NSObject[] { new NSString("superview"), new NSString("buttons") });
                center = NSLayoutConstraint.FromVisualFormat("H:|-[buttons]-|", NSLayoutFormatOptions.AlignAllCenterX, new NSDictionary(), btnsDictionary);
                alignLeft = NSLayoutConstraint.FromVisualFormat("H:|-[buttons]-" + spaceHorizontal + "-|", 0, new NSDictionary(), btnsDictionary);
                alignRight = NSLayoutConstraint.FromVisualFormat("H:|-" + spaceHorizontal + "-[buttons]-|", 0, new NSDictionary(), btnsDictionary);

                pageController.View.AddConstraints(center);
                pageController.View.AddConstraints(alignLeft);
                pageController.View.AddConstraints(alignRight);
                pageController.View.AddConstraints(NSLayoutConstraint.FromVisualFormat("V:[buttons]-|", 0, new NSDictionary(), btnsDictionary));
        ...
        void SetButtonsVisibility()
        {
            ...
            foreach (NSLayoutConstraint lc in center)
                lc.Active = !hidePrevElement && !hideNextElement;

            foreach (NSLayoutConstraint lc in alignLeft)
                lc.Active = hideNextElement && !hidePrevElement;

            foreach (NSLayoutConstraint lc in alignRight)
                lc.Active = hidePrevElement && !hideNextElement;

            prevTextBtn.Hidden = hidePrevElement;
            nextTextBtn.Hidden = hideNextElement;
        }
        ...
        void CleanUpButtons()
        {
            ...
            {
                prevTextBtn.TouchUpInside -= PrevBtn_TouchUpInside;
                prevTextBtn.RemoveFromSuperview();
                prevTextBtn.Dispose();
                prevTextBtn = null;
            }
            ...
            {
                nextTextBtn.TouchUpInside -= NextBtn_TouchUpInside;
                nextTextBtn.RemoveFromSuperview();
                nextTextBtn.Dispose();
                nextTextBtn = null;
            }
            if (buttons != null)
